...
contract Database {
    ...
    // User related DS
    mapping(address => mapping(uint256 => Data_Types.Emission[50])) user_emissions;
    ...
    function addOrganization(Data_Types.Organization memory newOrganization) public {
        organization_list[newOrganization.id] = newOrganization;
        emit UserRegistered(newOrganization.id, newOrganization.name);
    }
    ...
    function addProjectToList(uint256 id,Data_Types.Project memory args) public returns(bool) {
    ...
    function editProjectFromList(uint256 id,string memory name, string memory desc,string memory status) public returns(bool) {
        project_list[id].name = name;
        project_list[id].description = desc;
        project_list[id].status = status;
        emit ProjectEdited(project_list[id].id, project_list[id].name, project_list[id].org_id);
        return true;
    }

    function addProjectToCartDB(Data_Types.AddProjectToCartType memory args,string memory creditType) public returns (bool){
        if(!(bytes(user_cart_list[msg.sender][args.projectId].creditType).length > 0)){
            user_cart_id_list[msg.sender].push(args.projectId);
        }
        user_cart_list[msg.sender][args.projectId].creditType = creditType;
        user_cart_list[msg.sender][args.projectId].creditQuantityInCart = user_cart_list[msg.sender][args.projectId].isCartEmpty ? user_cart_list[msg.sender][args.projectId].creditQuantityInCart +  args.quantity : args.quantity;
    ...
    function transferCredits () public returns (bool){
        for(uint256 i=0;i<user_cart_id_list[msg.sender].length;i++){
            ...
            uint256 quantity= user_cart_list[msg.sender][projectId].creditQuantityInCart;
            project_list[projectId].creditQuantity = project_list[projectId].creditQuantity - quantity;
            string memory creditType = user_cart_list[msg.sender][projectId].creditType;
            if(!(bytes(user_credit_list[msg.sender][creditType].creditType).length > 0)){
                user_credit_type_list[msg.sender].push(creditType);
            }
            uint256 totalQuantity = user_credit_list[msg.sender][creditType].creditQuantityInPortfolio  + quantity;
            uint256 avgPrice = ((user_credit_list[msg.sender][creditType].creditQuantityInPortfolio *  user_credit_list[msg.sender][creditType].boughtAt) + (quantity * 100))/totalQuantity;
            user_credit_list[msg.sender][creditType].creditType = creditType;
            user_credit_list[msg.sender][creditType].creditQuantityInPortfolio = totalQuantity;
